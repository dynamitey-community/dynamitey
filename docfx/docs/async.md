...
That holds even in the case this library exists for: a method whose `Task<T>` or `ValueTask<T>` has a `T` that is **internal to another assembly**.
...
`Dynamic.InvokeMember` detects this specific case — a `Task<T>` or `ValueTask<T>` whose `T` is not visible outside its declaring assembly — and returns the result wrapped in an @Dynamitey.AwaitableResult instead of the raw task. An inaccessible `ValueTask<T>` is converted with `AsTask()` first so the wrapper stays Task-based.
...
A `Task<T>` or `ValueTask<T>` whose `T` is public, a nested public type, or a plain non-generic `Task` is returned **completely unchanged**. The wrapper only appears where it is needed.
