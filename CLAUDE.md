...
resolution required, admins included. A ruleset auto-requests Copilot on each
push and on drafts. That is a review, not a required status check. Conversation
...
`Build the site`. `Code coverage`, `Publish README badges`,
...
restricts the report to `[Dynamitey.Community]*`, which is what CI measures.
...
Two internals matter more than the file count:
...
argument count up to a fixed ceiling. Edit the `.tt`, not the `.cs`. The
hand-written default branch above that ceiling was the cause of #11
(`InvokeConstructor` above 14 arguments); that return-type mismatch is fixed.
The ceiling is still the emit path, and AOT still cannot run it (#27).
...
`Company` is `The dynamitey-community contributors`. `Copyright` retains
Ekon Benefits and adds the community line (#6). Upstream's copyright is
